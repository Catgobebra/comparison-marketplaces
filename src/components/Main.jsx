...
function Main() {
      ...
      if (tabs[0]?.url) {
        ...
        if (isOzon) {
          ...
          const isAlreadyAdded =
            sku && products.some((product) => product.article === sku);
  ...
  const getProductsInfo = async (arg) => {
    try {
      ...
      const promises = [arg].map((sku) =>
        fetch(`http://localhost:5000/api/Products/by-sku/${sku}`, {
          method: "GET",
          headers: {
            accept: "text/plain",
          },
  ...
  const openComparisonPage = async () => {
    ...
    try {
      ...
      const newProducts = [...result.result];
      dispatch(changeProducts(newProducts));
      chrome.storage.local.set({ myStoredArray: newProducts }).then(() => {});
  ...
  const addProduct = (currentState, currentSetState) => {
    const isAlreadyAdded = products.some(
      (product) => product.article === currentState.match(isCurrentSkuOzon)[1]
    );
      ...
      >
        <Box style={{ padding: "10px" }}>
          ...
          >
            ...
            <Button
              variant="contained"
              onClick={() => addProduct(currentLink, setCurrentLink)}
            >
              Добавить
            </Button>
            {showFab && isProduct && (
              <Fab
                onClick={() => addProduct(currentUrl, setCurrentUrl)}
                style={{ minWidth: "56px" }}
                color="primary"
                aria-label="add"
              >
